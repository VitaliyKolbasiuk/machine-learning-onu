...
def hyperparams_decision_tree(model, X_train, y_train, kf):
    tree_params = {
        'min_samples_split': np.arange(6, 18, 3),
        'max_depth': np.arange(2, 13, 2),
        'min_samples_leaf': [1, 2, 4, 6, 8],
    }
    tree_grid = GridSearchCV(model, tree_params, cv=kf, scoring='recall', n_jobs=-1, verbose=2)
...
def hyperparams_xgb(model, X_train, y_train, kf):
    xgb_params = {
...
def hyperparams_knn(model, X_train, y_train, kf):
    k_neighbors_params = {
